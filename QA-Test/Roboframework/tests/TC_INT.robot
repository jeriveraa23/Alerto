...
TC-INT-003 Conexión a PostgreSQL es estable y no hay pérdida de datos
    ...
    IF    len(${hist_list}) > 0
        ...
        Should Be Equal As Strings    ${curr_nivel}    ${hist_nivel}
        ...    msg=Inconsistencia: current=${curr_nivel} vs history[0]=${hist_nivel}
...
TC-INT-004 Airflow DAG weather_pipeline está activo y configurado
    ...
    ${resp}=    GET On Session    airflow_int    /api/v1/dags/weather_pipeline
    ...
    IF    ${status} == 200
        ...
        ${runs_resp}=    GET On Session    airflow_int
        ...    /api/v1/dags/weather_pipeline/dagRuns?limit=1
        ...    headers=${headers}    expected_status=any
    ...
    ELSE
        ...
        # Verificar indirectamente que el pipeline ha corrido (datos en BD)
